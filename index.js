...
let totalContributions = GAMES_JSON.reduce((accumulator, game)=> {
...
let numUnfunded = (unfunded.length > 1)? `Currently, there are ${unfunded.length} unfunded games.`: `Currently, there is ${unfunded.length} unfunded game.`;

// create a new DOM element containing the template string and append it to the description container
//descriptionContainer.appendChild(`<p>${numUnfunded}</p>`);
let pUnfundedString = document.createElement("p");
descriptionContainer.append(numUnfunded, pUnfundedString);
...
let [topPledge, runnerUp, ...restOfGames] = sortedGames;

// create a new element to hold the name of the top pledge game, then append it to the correct element
let pTop = document.createElement("p");
firstGameContainer.append(topPledge.name, pTop);

// do the same for the runner up item
let pRunnerUp = document.createElement("p");
secondGameContainer.append(runnerUp.name, pRunnerUp);